...
function sha_sign(hash_algo, data, key) {
    return b64(hash.hmac(hash_algo, new Buffer(key, 'hex')).digest(data));
}

function sha_verify(hash_algo, data, sign, key) {
    return b64(hash.hmac(hash_algo, new Buffer(key, 'hex')).digest(data)) === sign;
}
...
exports.sign = (alg, header, payload, key) => {
    if ((alg == '' || alg == null) &&
...
exports.verify = (jws, key, acceptAlgs) => {
    var a = jws.split(".");
