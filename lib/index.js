...
function pk_sign(pk_type, sign_alg, hash_algo, data, key) {
    var k;
    if (key.name == pk_type)
        k = key;
    else {
    ...
    var sign = hash_algo ? hash.digest(hash_algo, data).sign(k) : k.sign(data);

    if (this != 0)
        sign = formatEcdsa.derToJws(sign, getParamSize(k.keySize));

    return b64(sign);
}

function pk_verify(pk_type, sign_alg, hash_algo, data, sign, key) {
    var k;
    if (key.name == pk_type)
        k = key;
    else {
    ...
    if (this != 0) {
        ...
        k.alg = sign_alg;
        sign = formatEcdsa.jwsToDer(sign, getParamSize(k.keySize));
    }

    return hash_algo ? hash.digest(hash_algo, data).verify(k, sign) : k.verify(data, sign);
}
...
var signs = {
    ...
    'ES256': pk_sign.bind('P-256', 'EC', 'ECDSA', hash.SHA256),
    'ES384': pk_sign.bind('P-384', 'EC', 'ECDSA', hash.SHA384),
    'ES512': pk_sign.bind('P-521', 'EC', 'ECDSA', hash.SHA512),
    'ES256K': pk_sign.bind('P-256K', 'EC', 'ECDSA', hash.SHA256),
    'SM2SM3': pk_sign.bind('SM2', 'SM2', 'SM2', hash.SM3),
    'ES256-S': pk_sign.bind('P-256', 'EC', 'ECSDSA', hash.SHA256),
    'ES384-S': pk_sign.bind('P-384', 'EC', 'ECSDSA', hash.SHA384),
    'ES512-S': pk_sign.bind('P-521', 'EC', 'ECSDSA', hash.SHA512),
    'ES256K-S': pk_sign.bind('P-256K', 'EC', 'ECSDSA', hash.SHA256),
    'SM2SM3-S': pk_sign.bind('SM2', 'SM2', 'ECSDSA', hash.SM3),
    'EdDSA': pk_sign.bind('Ed25519', 'EC', 'EdDSA', 0)
};
...
var verifys = {
    ...
    'ES256': pk_verify.bind('P-256', 'EC', 'ECDSA', hash.SHA256),
    'ES384': pk_verify.bind('P-384', 'EC', 'ECDSA', hash.SHA384),
    'ES512': pk_verify.bind('P-521', 'EC', 'ECDSA', hash.SHA512),
    'ES256K': pk_verify.bind('P-256K', 'EC', 'ECDSA', hash.SHA256),
    'SM2SM3': pk_verify.bind('SM2', 'SM2', 'SM2', hash.SM3),
    'ES256-S': pk_verify.bind('P-256', 'EC', 'ECSDSA', hash.SHA256),
    'ES384-S': pk_verify.bind('P-384', 'EC', 'ECSDSA', hash.SHA384),
    'ES512-S': pk_verify.bind('P-521', 'EC', 'ECSDSA', hash.SHA512),
    'ES256K-S': pk_verify.bind('P-256K', 'EC', 'ECSDSA', hash.SHA256),
    'SM2SM3-S': pk_verify.bind('SM2', 'SM2', 'ECSDSA', hash.SM3),
    'EdDSA': pk_verify.bind('Ed25519', 'EC', 'EdDSA', 0)
};
