...
function pk_sign(pk_algo, hash_algo, data, key) {
    var k;
    if (key.name == pk_algo)
        k = key;
    else {
        ...
        k.importKey(key);
        if (k.name !== pk_algo)
            throw 'private key algorithm should be ' + pk_algo;
    }

    var sign = hash.digest(hash_algo, data).sign(k);

    if (this != 0) {
        if (k.keySize != this)
            throw 'private key size should be ' + this;
        sign = formatEcdsa.derToJws(sign, getParamSize(this));
    }

    return b64(sign);
}

function pk_verify(pk_algo, hash_algo, data, sign, key) {
    var k;
    if (key.name == pk_algo)
        k = key;
    else {
        ...
        k.importKey(key);
        if (k.name !== pk_algo)
            throw 'public key algorithm should be ' + pk_algo;
    }

    sign = new Buffer(sign, 'base64');
    if (this != 0) {
        if (k.keySize != this)
            throw 'public key size should be ' + this;
        sign = formatEcdsa.jwsToDer(sign, getParamSize(this));
    }

    return hash.digest(hash_algo, data).verify(k, sign);
}
...
var signs = {
    ...
    'ES256': pk_sign.bind(256, 'EC', hash.SHA256),
    'ES256K': pk_sign.bind(256, 'EC', hash.SHA256),
    'ES384': pk_sign.bind(384, 'EC', hash.SHA384),
    'ES512': pk_sign.bind(521, 'EC', hash.SHA512),
    'SSM3': pk_sign.bind(256, 'SM2', hash.SM3)
};
...
var verifys = {
    ...
    'ES256': pk_verify.bind(256, 'EC', hash.SHA256),
    'ES256K': pk_verify.bind(256, 'EC', hash.SHA256),
    'ES384': pk_verify.bind(384, 'EC', hash.SHA384),
    'ES512': pk_verify.bind(521, 'EC', hash.SHA512),
    'SSM3': pk_verify.bind(256, 'SM2', hash.SM3)
};

exports.ALGORITHMS = [
    'HS256', 'HS384', 'HS512', 'HSM3',
    'RS256', 'RS384', 'RS512',
    'ES256', 'ES256K', 'ES384', 'ES512', 'SSM3'
];
